...
> 范围：Android 语音录制与确认、FastAPI ASR、查询改写、Dify 问答、本地 TTS 播报
...
- Android 端已接入本地 `TextToSpeech`，回答页支持“朗读回答”和“停止朗读”。
...
- 后端配置已补充 `DASHSCOPE_API_KEY`、`QWEN_BASE_URL`、`QWEN_MODEL`、`ASR_MODEL`、`ASR_TIMEOUT_SECONDS` 等环境变量。
